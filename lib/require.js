(function(root) {
	...
	Loader.next = function() {
		...
		if (!some(Loader.LOADERS, function(loader) {
			...
			some(Loader.LOADERS, function(loader) {
				return loader.cycle() && loader.next();
			});
		}
	...
	Loader.replaceContentLoaded = function() {
		if (!root.window) {
			...
			// Ugh... IE fires this early when adding dynamic tags, which breaks
			// testem/jasmine.
			if (window.onload) { Loader.ready(window.onload); }
			window.onload = null;
	...
	Script.prototype = {
		...
		bind: function() {
			...
			function loaded(event) {
				if (script.absolute !== absolute) { return; }
				script.onload();
	...
	Script.current = function() {
		if (document.currentScript) {
			return Script.tag(document.currentScript,
				Loader.using || Loader.default);
		}

		var tags		= document.getElementsByTagName('script'),
			tag			= tags[tags.length - 1];

		var current = Script.tag(tag, Loader.using || Loader.default);
		return current;
	};
