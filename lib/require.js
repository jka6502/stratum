(function(root) {
	...
	Loader.prototype = {
		...
		define: function(id, depends, callback) {
			...
			each(depends, function(depend) {
				var dscript = Script.url(depend, this);
	...
	Loader.ready = function(callback) {
		if (Loader.loaded) {
			...
			Loader.load.push(Loader.onready(callback));
		}
	};
	...
	Script.url = function(url, loader) {
		...
		var relative	= url.charAt(0) === '.',
			...
			base		= relative ? URL.path(current.absolute) : loader.paths[0],
	...
	// Stratum specific helper, for load state trigger.
	root.require.ready = function(callback) {
