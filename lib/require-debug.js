(function(root) {
	...
	function s(item) {
		if (item instanceof stratum.Script) {
			return '[' + item.loader.id + '] ' + item.loader.relative(item.absolute || item.id || '');
	...
	intercept(Loaded, 'loaded', LOADED, function() {
		return ['DOCUMENT READY'];
	});

	document.addEventListener('DOMScriptsLoaded', Loaded.loaded);
